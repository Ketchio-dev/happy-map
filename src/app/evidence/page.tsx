...
import { busiest, type LoggedAlert } from "@/lib/outages";
...
const when = (iso: string) => new Date(iso).toLocaleString("en-CA", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit", timeZone: "America/Toronto" }).replace(".m.", "m");
/** the map, step-free, walk+subway, from Union to the named station, as things were at that instant */
const replayHref = (at: string, station: string) => { const c = coordsFor(station); return `/?at=${encodeURIComponent(at.replace(/\.\d{3}Z$/, "Z"))}&mode=stepfree&walk=0&tab=live&from=-79.38060,43.64530,Union%20Station${c ? `&to=${c[0].toFixed(5)},${c[1].toFixed(5)},${encodeURIComponent(station + " Station")}` : ""}`; };
...
export default async function Evidence() {
  ...
  const worst = out ? busiest(out.alerts, Date.parse(out.summary.logging.first), Date.parse(out.summary.logging.last)) : null;
  const worstStation = worst && out ? out.alerts.find((a) => a.type === "Elevator" && Date.parse(a.feedStart ?? a.first) <= worst.t && (!a.end || Date.parse(a.end) > worst.t))?.station : null;
  return (
    <main className="mx-auto max-w-4xl space-y-8 p-6 text-[13.5px] leading-relaxed text-ink-soft">
      ...
      <section>
        ...
        {out ? (
          <>
            ...
            {worst && worst.n > 0 && (
              <p className="mt-3 rounded-lg border border-line bg-sunk p-2.5 text-[12.5px] text-ink-soft">The log can be replayed on the map: every route is then costed with the elevators that were out at that moment. Most at once so far: <span className="tnum font-semibold text-ink">{worst.n}</span> elevators at {when(new Date(worst.t).toISOString())}. <Link href={replayHref(new Date(worst.t).toISOString(), worstStation ?? "Bloor-Yonge")} className="font-medium text-ink underline decoration-line underline-offset-2 hover:decoration-ink">Replay that moment →</Link></p>
            )}
            <table className="mt-4 w-full text-xs">
              <thead><tr className="border-b border-line text-left text-muted"><th className="py-1 font-normal">Station</th><th className="font-normal">Unit</th><th className="font-normal">Kind</th><th className="font-normal">Cause</th><th className="font-normal">Duration (h)</th><th className="font-normal">Status</th><th className="font-normal">Map</th></tr></thead>
              <tbody>{out.alerts.filter((a) => a.type === "Elevator").sort((a, b) => (b.sinceFeedStartHours ?? 0) - (a.sinceFeedStartHours ?? 0)).map((a) => (
                <tr key={a.id} className="border-b border-line"><td className="py-1">{a.station}</td><td>{a.code}</td><td>{a.unplanned ? "breakdown" : "planned"}</td><td>{a.causeDesc ?? "—"}</td><td>{a.sinceFeedStartHours ?? "—"}</td><td>{a.ongoing ? <span className="text-alert">out now</span> : "restored"}</td><td><Link href={replayHref(new Date(Math.max(Date.parse(a.first), Date.parse(a.feedStart ?? a.first))).toISOString(), a.station)} className="text-muted underline decoration-line underline-offset-2 hover:text-ink">replay</Link></td></tr>
              ))}</tbody>
            </table>
          </>
      ...
      <section>
        ...
        <ul className="mt-1 list-disc space-y-1 pl-5 text-ink-soft">
          ...
          <li><strong>Subway and light rail:</strong> TTC GTFS (Lines 1, 2, 4, 5 and 6), median inter-station times, legs drawn along the recorded track geometry, stations linked to OpenStreetMap entrances. In step-free mode a station is unusable while the TTC feed lists an elevator out of service there; the same trip is also costed with every elevator working, and the difference is shown on the card.</li>
          <li><strong>Weather:</strong> Environment and Climate Change Canada city page feed; heat or cold warnings switch the default mode. Cloud cover now, from Open-Meteo, scales the shade penalty by 1 − 0.75·N<sup>3.4</sup> (Kasten and Czeplak, 1980), so an overcast sky leaves a quarter of the weight and a clear one all of it.</li>
          <li><strong>Cost model:</strong> indoor mode weights outdoor time ×2.5 and covered ×1.3; shade mode weights outdoor time ×(1 + 1.8 × sky × sun fraction). Roadway with no sidewalk ×1.35, loose surface ×1.15, steep grades up to ×1.24 — each harsher in step-free mode. Stairs ×1.6 for walkers and forbidden in step-free mode. The fastest feasible route is always computed alongside for comparison. Every multiplier is at least 1, which is what keeps the A* heuristic admissible; <code>tests/</code> checks that against the packed graph on every push.</li>
        </ul>
      </section>
    </main>
  );
