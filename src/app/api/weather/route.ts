...
const URL_SKY = "https://api.open-meteo.com/v1/forecast?latitude=43.65&longitude=-79.38&current=cloud_cover,is_day&timezone=America%2FToronto";
...
async function sky(): Promise<{ cloud: number | null; isDay: boolean | null }> {
  try {
    const r = await fetch(URL_SKY, { next: { revalidate: 900 }, headers: { "user-agent": "toronto-exposure-router/0.1" } });
...
export async function GET() {
  try {
    const [res, { cloud, isDay }] = await Promise.all([fetch(URL_ECCC, { next: { revalidate: 300 }, headers: { "user-agent": "toronto-exposure-router/0.1" } }), sky()]);
    ...
    const out: Weather = { ok: true, fetched: new Date().toISOString(), station: str(it.properties.name) ?? "Toronto", temp, humidex, windChill, condition: str(cc.condition), warnings, suggested: { cold, heat }, reason, cloud, sky: skyFactor(cloud), isDay };
