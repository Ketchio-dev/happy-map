#!/usr/bin/env node
// Single-shot version of the poller, for GitHub Actions. Appends to data/ttc-alerts/<date>.jsonl
...
const res = await fetch(URL_ALERTS, { headers: { "user-agent": "happy-map/0.1 (research logger; github actions)" } });
...
await appendFile(file, JSON.stringify({ t: now.toISOString(), feedUpdated: j.lastUpdated, total: j.total, nAcc: acc.length, changed: true, source: "actions", accessibility: acc }) + "\n");
