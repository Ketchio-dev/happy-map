...
for (const [trip, list] of stopTimes) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (i > 0) { const ka = stationOf(list[i-1].stop); if (!ka || ka === key) continue; const dt = list[i].t - list[i-1].t; const ek = [ka, key].sort().join("|") + "|" + line; if (!edges.has(ek)) edges.set(ek, []); edges.get(ek).push(dt); }
  }
}
const out = {
  ...
  edges: [...edges.entries()].map(([k, ts]) => { const [a, b, line] = k.split("|"); ts.sort((x, y) => x - y); return { a, b, line, time_s: ts[Math.floor(ts.length / 2)] }; }),
};
